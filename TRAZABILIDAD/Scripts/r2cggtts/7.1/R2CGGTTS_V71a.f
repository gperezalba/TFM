...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     2  ichoice_sys,mjdtrk,IOE,ck,elv,azth,ch,fr,hc,pprn_gal(5000),
     2  pprn_gps(5000),colGPSP1,colGPSP2,colGPSC1,colGLOP1,colGLOP2,
     2  colGALE1,colGALE5,fact,snr(45),nb_obs(5),secn,heure,binr(3,2),
     1  iGP1,iGP2, iGC1,iRP1,iRP2,iRC1,iE1,iE5,n1(5),colGLOC2

     
      real*8 pi,vitlum,schtime(300),sec,mjd,mjddep, a0,a1,corrgeom,     
     1  brorb_gps(5000,7,4),corprev,alpha,we,ds,djul,last,trel,tau,     
     1  deltatk,tgd,brorb_gal(5000,7,4),antpos(3),x(3),xs(3),lla(3),
     1  Az,El,moment,timefirst,timestop,timenav,toc,trc,Ttr,tropo(26),           
     1  mdtr,smdt,stdv, brorb(5000,7,4),svclk_gps(5000,3),iono(26),    
     1  mdio,smdi,measiono(26,45),isg,svclk_gal(5000,3), refsv(26),    
     ...
     4  p1valobs(45),datenav_gps(5000),bias(40),NS,DeltaN,mf,zpd,
     4  NSlog,datenav_gal(5000),factor(3,2),freq(3,2),ffactor(3,2),
     4  lambda(3,2),k1(3),k2(3),timenav1,datenav1(5000),diftime,
...
C    GLONASS ADDITION 	 
     ...
     1    recdelP1g,recdelP2g
      ...
      character*2 class,schcl(5000),schclm(5000), brol, year,month,day
...
1900   iv=1             ! if end of file in the first file, needed when used with hourly files. 
 ...
 897   deltaTK=0.
       ...
       if(systime.ne.'GPS')call leapscorr(yy,mm,dd,hh,min,sec,leapsec)
       yy=yy-2000
 ...
 887    read(LU,'(a500)')string
	...
	if(syst.eq.'G')then	
       ...
       elseif(syst.eq.'R')then	
        ...
        col2(iii)=colGLOP2
        if(colGLOP2.eq.0)col1(iii)=colGLOC2
...
c   Discard the block either DVS=1 or HS=1       
	  ...
	  if(binr(2,2).eq.1)goto 110
     
          diftime=trc-brorb_gal(jkl,7,1)
     1	             +604800*(WN-brorb_gal(jkl,5,3))
          if(dabs(diftime).lt.dabs(difsave).and.diftime.gt.0.and.
     1                   dabs(diftime).le.7200)then
...
625   format(A7,' = ',f6.1,' ns (GAL E1), ',
     1        f6.1,' ns (GAL E5a',a1,')',5x,'CAL_ID = ',A20)
...
c if IRCV_CLOCK=1 : read the rcv clock applied each time, and correct for it.
  ...
  175 format(a1,1x,i4,2x,12(1x,a3))
...
C		toc = date de l'ephemeride de l'IOE recherché (ce n'est plus un vecteur)
C		sp3 = vecteur des eph.
c       xs : position (sortie)
c       vit: vitesse (sortie)
c       exists = test logique qui vérifie que les positions ont bien été calculées
c       LULOG = code fichier log
c       jkl = indice de l'ephemeride à utiliser, déterminé avec le code suivant:
c             ( datenavglo (jkl) = date en MJD de l'éphéméride, (= toc qui est exprimé en GPSweek))
ccc	             do jkl=1,1000
ccc                if(jkl==1) then
